...
- **`app/src/kb-data.mjs` is AUTO-GENERATED** from `data/**` by `npm run build-data`. After ANY change under `data/`, run `build-data` **and commit the regenerated `kb-data.mjs` in the same change** — otherwise `npm start` (no build) serves stale data and the repo drifts from prod.
