...
1. **[Goal 4] Wins & PR celebration.** — *PARTIALLY SHIPPED (Waves 79, 81).* Done: reusable pure
   ...
   banner; bonus XP for a PR (Wave 81 — `adherence.mjs` `xpAndLevel` replays `detectPersonalRecords`
   over each session's own prior history, +50 XP per PR, so a PR session always out-levels an
   identical non-PR one and the reward can never disagree with the recap banner). **Remaining
   slices (build next):** (b) an in-*player* PR moment (celebrate mid-session when a set beats a
   best, not only in the recap); (c) a persistent "wins"/PR feed + PR count (a lookback surface —
   progress-dopamine).
