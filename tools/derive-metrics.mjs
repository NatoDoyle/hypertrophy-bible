#!/usr/bin/env node
// Derive-don't-ask engine for The Hypertrophy Bible.
//
// Turns raw, low-burden primitives (workout sets, bodyweight, daily check-ins) into the
// high-signal features a personalization model actually needs — WITHOUT asking users to
// self-report them. Everything here is computed from objective inputs + the KB's own
// exercise->muscle map and volume landmarks.
//
// Pure functions are exported for testing. Run directly for a demo over examples/.
...
// ---------------------------------------------------------------------------
// KB data loaders (the exercise->muscle map and volume landmarks are the bridge
// between the knowledge base and the learning model).
// ---------------------------------------------------------------------------
